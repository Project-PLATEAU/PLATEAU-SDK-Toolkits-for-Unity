...
namespace ProceduralToolkit
{
    ...
    public partial class MeshDraft
    {
        ...
        public MeshDraft Add(MeshDraft draft)
        {
            ...
            foreach (Material material in draft.materials.Where(material => !materials.Contains(material)))
            {
        ...
        /// Adds a quad to the draft
        /// </summary>
        public MeshDraft AddQuad(Vector3 origin, Vector3 width, Vector3 height, Vector2 uvScale, bool calculateNormal, bool generateUV)
        {
            if (generateUV)
            {
                uv.Add(new Vector2(origin.x * uvScale.x, origin.y * uvScale.y));
                uv.Add(new Vector2(origin.x * uvScale.x, (origin + height).y * uvScale.y));
                uv.Add(new Vector2((origin + width).x * uvScale.x, (origin + height).y * uvScale.y));
                uv.Add(new Vector2((origin + width).x * uvScale.x, origin.y * uvScale.y));
            }
        ...
        /// </summary>
        public MeshDraft AddQuad(Vector3 vertex0, Vector3 vertex1, Vector3 vertex2, Vector3 vertex3, bool calculateNormal,
            Vector3 origin, Vector3 width, Vector3 height, Vector2 uvScale, bool generateUV)
        {
            if (generateUV)
            {
                uv.Add(new Vector2(origin.x * uvScale.x, origin.y * uvScale.y));
        ...
        /// </summary>
        public MeshDraft AddFlatQuadBand(IReadOnlyList<Vector3> lowerRing, IReadOnlyList<Vector3> upperRing, Vector2 uvScale, bool generateUV)
        {
            if (lowerRing.Count != upperRing.Count)
            {
                throw new ArgumentException("Array sizes must be equal");
            ...
            for (int i = 0; i < ringCount - 1; i++)
            {
                ...
                {
                    float uvX0 = i * uvStep;
                    float uxX1 = (i + 1) * uvStep;
                    var uv00 = new Vector2(uvX0, 0);
        ...
        /// <summary>
        /// Paints draft polygon with <paramref name="material"/>
        /// </summary>
        public MeshDraft Paint(Material material, bool generateDummyUV = false)
        {
            colors.Clear();
            for (int i = 0; i < vertices.Count; i++)
            {
        ...
        /// <summary>
        /// Paints draft vertices with <paramref name="color"/> and <paramref name="material"/>
        /// </summary>
        public MeshDraft Paint(Color color, Material material, bool generateDummyUV = false)
        {
            colors.Clear();
            for (int i = 0; i < vertices.Count; i++)
            {
