...
namespace PlateauToolkit.Sandbox.Runtime.PlateauSandboxBuildingsLib
{
    ...
    public class CompoundMeshDraft : IEnumerable<MeshDraft>
    {
        ...
        public void MergeDraftsWithTheSameName()
        {
            ...
            {
                MeshDraft merged = MergeDraftsWithName(meshDrafts[i].name);
        ...
        private MeshDraft MergeDraftsWithName(string draftName)
        {
            ...
            for (int i = 0; i < meshDrafts.Count; i++)
            {
                MeshDraft meshDraft = meshDrafts[i];
        ...
        {
            meshDrafts.Sort((a, b) => string.Compare(a.name, b.name, StringComparison.Ordinal));
        }
