...
配置する際のオブジェクトのデフォルトの正面方向は「配置位置」の設定により異なります。また、クリックにて配置する際にドラッグをすることで、配置する際の正面方向を変更できます。【要確認】「また、・・」の文について、180行目に移動していただけますでしょうか。（添付している動画にてドラッグしている操作が映っているため）
