...
<img width="700" alt="asset_filter" src="../Documentation~/Sandbox Images/asset_filter.png">
...
`SHP、CSVファイルを読み込む` ボタンを押下し、シェープファイル(.shp)を選択することで読み込みできます。
...
シェープファイルに紐づけされている、属性情報をアセット種別として選択できます。
...
都市モデルインポート時に原点の高さを0としてインポートする場合、3D都市モデルは「東京湾平均海面を基準とする標高」に配置されるため、CSVには配置したい標高を記載すると、意図通りの場所に配置できます。
...
編集して保存したCSVファイルを `SHP、CSVファイルを読み込む` ボタンから選択し読み込みできます。
...
`地面に配置` を選択することで、高さ情報を無視して地面に配置することが可能です。ただし `地面に配置` の場合、該当の緯度、経度にコライダーが存在しない場合はアセットは配置されません。
...
`建造物設定` の「建造物タイプ」を「マンション」に設定するとマンション独自のパラメータを変更できます。本項目は建物タイプによって表示されるパラメータが異なります。
